...
import subprocess
from modules.config import EFS_DIR
...
def path_to_uri(path, scheme="", domain=""):
    return scheme + domain + str(path)
...
def get_data(_config):
    ...
    scrap_data_sh = f"./bash_scripts/load_{dataset}_datasets.sh"
    subprocess.run(f"bash {scrap_data_sh}", shell=True, check=True)
